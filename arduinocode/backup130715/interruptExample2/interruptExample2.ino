...
void interruptE(pin1,pin2,counter) {
  E1A_state = digitalRead(pin1);
  E1B_state = digitalRead(pin2);
  E_turning = false;
  if (E1A_state && E1B_state){ // if both pins are 1, rotation is finished so reset turn signal
    E1_turning=false;
    return;
  }
  elif (E1A_state==0 && E1B_state==0){ // the encoder has reached the halfway point meaning we are turning
    E1_turning=true;
    return;
  }
  else{
...
void setup() {
  ...
  enableInterrupt(E1A, interruptE(E1A,E1B,E1_count), CHANGE);
